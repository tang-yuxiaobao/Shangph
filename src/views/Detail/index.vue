        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            ...
            <div class="cartWrap">
              ...
              <div class="add">
                <!-- 以前咱们的路由跳转:从A路由跳转到B路由；这里：在加入购物车，要进行路由跳转之前，发请求
...
export default {
  ...
  computed: {
    ...
    // 给子组件的数据
    skuImageList() {
      // 如果服务器的数据没有回来，skuInfo至少应该是一个空对象
      return this.skuInfo.skuImageList || [];
    },
  },
  methods: {
    // 点击产品的某个属性值则该属性值高亮（排他）
    ...
    // 加入购物车的回调函数
    async addShopCar() {
      // 1 发请末/派发action---将产品加入到数据库（通知服务器)
      // this.$store.dispatch("addOrUpdateShopCart")相当于调用了仓库中的 addOrUpdateShopCart 函数
      // 捕获 addOrUpdateShopCart 函数的结果
      try {
        // 成功了怎么样：路由跳转
        // 这是一个promise对象；await：等待 addOrUpdateShopCart 的成功或失败
